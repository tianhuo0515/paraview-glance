...
/**
 * Automatically apply scalar coloring to a source's representations.
 * Detects point or cell scalar arrays and sets ColorBy on Geometry representations.
 */
function autoApplyColorBy(proxyManager, source) {
  const dataset = source.getDataset();
  if (!dataset) {
    return;
  }

  // Only apply to polygon data (vtkPolyData), unstructured grid, etc.
  // Skip image data (handled by volume rendering)
  if (dataset.isA && dataset.isA('vtkImageData')) {
    return;
  }

  const pointScalars =
    dataset.getPointData && dataset.getPointData().getScalars();
  const cellScalars = dataset.getCellData && dataset.getCellData().getScalars();
  ...
  // Prefer point data scalars
  let arrayName = null;
  let arrayLocation = null;
  if (pointScalars) {
    arrayName = pointScalars.getName();
    arrayLocation = 'pointData';
  } else if (cellScalars) {
    arrayName = cellScalars.getName();
  ...
  // Apply ColorBy to all Geometry representations of this source
  const representations = proxyManager
    .getRepresentations()
    .filter((r) => r.getInput() === source);

  let hasGeometryRep = false;
  representations.forEach((rep) => {
    if (rep.getProxyName() === 'Geometry') {
      hasGeometryRep = true;
      rep.setColorBy(arrayName, arrayLocation);
      // VTK.js 28+ : setScalarVisibility is deprecated/removed
      // setColorBy with array name implicitly enables scalar coloring
      try {
        if (typeof rep.setScalarVisibility === 'function') {
          rep.setScalarVisibility(true);
        }
      } catch (_e) {
        // ignore: new API handles this via setColorBy
      }
    }
  });

  if (hasGeometryRep) {
    // Set Rainbow color map preset (ParaView rainbow)
    const lutProxy = proxyManager.getLookupTable(arrayName);
    if (lutProxy) {
      lutProxy.setPresetName('rainbow');
    }
  }
}
  ...
  for (let i = 0; i < readers.length; i += 1) {
    ...
    if (reader || dataset) {
      ...
      if (source) {
        ...
        // Auto-apply scalar coloring (contour plot / 云图) for VTK models
        autoApplyColorBy(proxyManager, source);
